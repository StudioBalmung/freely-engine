...
EditorApp::EditorApp()
    : Engine(Freely::EngineConfig::Defaults())
{
}

void EditorApp::OnInit() {
    FL_INFO("Freely Editor 0.4.4 initialized!");
    ...
    ConsolePanel::InstallSink();   // Wire spdlog → console UI
    m_ToolbarPanel = std::make_unique<ToolbarPanel>(&m_Context);

    // The viewport only reads the editor camera's matrices to place the
    // ImGuizmo overlay; it never owns or updates the camera itself.
    m_ViewportPanel->SetCamera(&m_EditorCamera);
...
void EditorApp::OnRender() {
    ...
    // Clear background and ensure viewport framebuffer has a texture even when no project is loaded
    m_ViewportFB->Bind();
...
void EditorApp::DrawMenuBar() {
    ...
    if (ImGui::MenuItem("Open Project")) {
        ...
        if (ImGui::BeginMenu("Edit")) {
            char undoLabel[64];
            char redoLabel[64];
            std::snprintf(undoLabel, sizeof(undoLabel), "Undo %s", m_Context.CanUndo() ? m_Context.PeekUndoName() : "");
            std::snprintf(redoLabel, sizeof(redoLabel), "Redo %s", m_Context.CanRedo() ? m_Context.PeekRedoName() : "");

            if (ImGui::MenuItem(undoLabel, "Ctrl+Z", false, m_Context.CanUndo())) {
                m_Context.Undo();
            }
            if (ImGui::MenuItem(redoLabel, "Ctrl+Y", false, m_Context.CanRedo())) {
                m_Context.Redo();
            }
            ImGui::EndMenu();
        }
...
void EditorApp::HandleViewportCamera(float dt) {
    ...
    if (vpSize.x > 0 && vpSize.y > 0) {
        if (vpSize.x != m_ViewportFB->GetSpec().Width || vpSize.y != m_ViewportFB->GetSpec().Height) {
