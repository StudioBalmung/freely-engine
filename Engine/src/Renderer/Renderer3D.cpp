...
struct State {
    ...
    uint32_t ViewportW = 1600;
    uint32_t ViewportH = 900;
...
void Renderer3D::BeginScene(const Camera& camera, const SceneLights& lights) {
    ...
    for (int i = 0; i < nPt; i++) {
        ...
        dst.Color       = glm::vec4(src.Color, src.Intensity); // w = intensity
...
std::shared_ptr<Shader> Renderer3D::BuildPBRShader() {
    return Shader::CreateFromFileWithFallback("shaders/pbr.vert", "shaders/pbr.frag", R3D::kPBR_Vert, R3D::kPBR_Frag);
}
std::shared_ptr<Shader> Renderer3D::BuildShadowShader() {
    return Shader::CreateFromFileWithFallback("shaders/shadow.vert", "shaders/shadow.frag", R3D::kShadow_Vert, R3D::kShadow_Frag);
}
std::shared_ptr<Shader> Renderer3D::BuildSkyboxShader() {
    return Shader::CreateFromFileWithFallback("shaders/skybox.vert", "shaders/skybox.frag", R3D::kSkybox_Vert, R3D::kSkybox_Frag);
}
std::shared_ptr<Shader> Renderer3D::BuildGridShader() {
    return Shader::CreateFromFileWithFallback("shaders/grid.vert", "shaders/grid.frag", R3D::kGrid_Vert, R3D::kGrid_Frag);
}
