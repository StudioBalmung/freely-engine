// Freely Engine - LuaEngine implementation
...
static std::unordered_map<std::string, std::filesystem::file_time_type> s_ScriptWriteTimes;
...
static int lua_IsKeyDown(lua_State* L) {
    ...
    lua_pushboolean(L, Engine::Get().GetInput().IsKeyDown(key));
    return 1;
}
static int lua_IsMouseDown(lua_State* L) {
    int btn = (int)lua_tointeger(L, 1);
    lua_pushboolean(L, Engine::Get().GetInput().IsMouseButtonDown(btn));
    return 1;
}
static int lua_GetMouseDelta(lua_State* L) {
    glm::vec2 delta = Engine::Get().GetInput().GetMouseDelta();
...
void LuaEngine::OnSceneStart(Scene& scene) {
    ...
    for (auto e : view) {
        ...
        std::string resolvedPath = AssetManager::ResolveAbsolute(sc.ScriptPath);
        if (!LoadScript(resolvedPath)) continue;

        try {
            if (std::filesystem::exists(resolvedPath)) {
                s_ScriptWriteTimes[sc.ScriptPath] = std::filesystem::last_write_time(resolvedPath);
            }
        } catch (...) {}
...
void LuaEngine::OnSceneUpdate(Scene& scene, float dt) {
    ...
    for (auto e : view) {
        ...
        // Check for script file changes (Hot-reload)
        try {
            std::string resolvedPath = AssetManager::ResolveAbsolute(sc.ScriptPath);
            if (std::filesystem::exists(resolvedPath)) {
                auto writeTime = std::filesystem::last_write_time(resolvedPath);
                if (s_ScriptWriteTimes.find(sc.ScriptPath) == s_ScriptWriteTimes.end()) {
                    s_ScriptWriteTimes[sc.ScriptPath] = writeTime;
                } else if (s_ScriptWriteTimes[sc.ScriptPath] != writeTime) {
                    FL_ENGINE_INFO("Hot-reloading script: {}", sc.ScriptPath);
                    LoadScript(resolvedPath);
                    s_ScriptWriteTimes[sc.ScriptPath] = writeTime;
                }
            }
        } catch (...) {
            // Ignore potential file locks during IDE saves
        }
