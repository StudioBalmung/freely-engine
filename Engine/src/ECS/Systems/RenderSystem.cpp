...
void RenderSystem::OnLateUpdate(Scene& scene, float deltaTime) {
    ...
    int vpW = Engine::Get().GetWindow().GetWidth();
    int vpH = Engine::Get().GetWindow().GetHeight();
    ...
    // -- 5. Submit all renderable entities ---------------------------------
    {
        ...
        for (auto e : meshView) {
            ...
            if (!mesh) {
                if (mr.MeshHandle != 0) {
                    mesh = AssetManager::GetMesh(mr.MeshHandle);
                } else if (!mr.MeshAsset.empty()) {
                    mesh = AssetManager::GetMesh(mr.MeshAsset);
                }
            }
            ...
            if (reg.any_of<MaterialComponent>(e)) {
                ...
                mat->TwoSided = mc.TwoSided;
                mat->AlphaCutoff = mc.AlphaCutoff;

                if (!mc.AlbedoMapPath.empty()) {
                    mat->AlbedoMap = AssetManager::GetTexture(mc.AlbedoMapPath);
                }
                if (!mc.NormalMapPath.empty()) {
                    mat->NormalMap = AssetManager::GetTexture(mc.NormalMapPath);
                }
                if (!mc.MetallicRoughnessMapPath.empty()) {
                    mat->MetallicRoughnessMap = AssetManager::GetTexture(mc.MetallicRoughnessMapPath);
                }
            } else {
