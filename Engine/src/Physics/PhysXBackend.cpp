// PhysX backend stub. Compiled only when FREELY_PHYSICS_PHYSX is defined.
...
class PhysXBackend3D final : public IPhysicsBackend3D {
public:
    bool Initialize(const PhysicsConfig&) override {
        FL_ENGINE_WARN("PhysX backend registered but not yet fully implemented.");
        return false;
    }
    void Shutdown() override {}
    const char*       GetName() const override { return "PhysX"; }
    PhysicsBackend3D  GetType() const override { return PhysicsBackend3D::PhysX; }
...
std::unique_ptr<IPhysicsBackend3D> CreatePhysXBackend3D() {
    return std::make_unique<PhysXBackend3D>();
}

} // namespace Freely::Physics

#endif // FREELY_PHYSICS_PHYSX
