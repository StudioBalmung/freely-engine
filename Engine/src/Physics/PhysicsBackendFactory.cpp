...
std::unique_ptr<IPhysicsBackend3D> PhysicsBackendFactory::Create3D(PhysicsBackend3D type) {
    switch (type) {
        case PhysicsBackend3D::AsterCore:
#ifdef FREELY_PHYSICS_JOLT
            FL_ENGINE_WARN("AsterCore backend disabled. Falling back to Jolt.");
            return CreateJoltBackend3D();
#elif defined(FREELY_PHYSICS_PHYSX)
            FL_ENGINE_WARN("AsterCore backend disabled. Falling back to PhysX.");
            return CreatePhysXBackend3D();
#else
            FL_ENGINE_WARN("AsterCore backend disabled. No 3D physics backend available.");
            return nullptr;
#endif
        case PhysicsBackend3D::Jolt:
            ...
            return CreateJoltBackend3D();
#else
            FL_ENGINE_WARN("Jolt backend not compiled in. Falling back to nullptr.");
            return nullptr;
#endif
        case PhysicsBackend3D::PhysX:
            ...
            return CreatePhysXBackend3D();
#else
            FL_ENGINE_WARN("PhysX backend not compiled in. Falling back to nullptr.");
            return nullptr;
#endif
...
std::vector<PhysicsBackend3D> PhysicsBackendFactory::AvailableBackends3D() {
    std::vector<PhysicsBackend3D> list;
#ifdef FREELY_PHYSICS_JOLT
