...
// Freely Engine - ECS Registry
// Thin wrapper around entt::registry with Freely-specific helpers.

// UUID.h already includes Components.h - only one include needed here.
#include "UUID.h"
...
public:
    ...
    // ─── Views ───────────────────────────────────────────────────────────
    // NOTE: entt::registry::view() is NOT const in EnTT v3.
    // Only the non-const overload is provided; use it via non-const Registry references.
    ...
    // m_Registry is mutable because entt::registry::view() is not const in EnTT v3.
    ...
    // NOTE: EnTT v3 removed registry::each(). Use IDComponent view as a proxy
    // to enumerate all alive entities (every entity has IDComponent by construction).
    ...
    /// Get entity count.
    // NOTE: EnTT v3 removed storage().in_use(). Use registry::size() instead.
    ...
    void ForEachChild(entt::entity parent, const std::function<void(entt::entity)>& fn) const {
        if (!m_Registry.valid(parent)) return;
        const auto& rel = m_Registry.get<RelationshipComponent>(parent);
        auto child = rel.FirstChild;
        while (child != entt::null) {
            fn(child);
    ...
    // Alias used by RenderSystem and other subsystems that need the raw registry.
    entt::registry&       GetEnttRegistry()       { return m_Registry; }
    const entt::registry& GetEnttRegistry() const { return m_Registry; }

private:
    // mutable: EnTT v3 registry::view(), each(), storage() are not const-qualified.
    // All const query helpers need non-const access to the underlying registry.
